...
function WeddingTimeline({ weddingData, onUpdate, compact }) {
  ...
  const generatePDF = () => {
    ...
    // Page 1: Wedding Day Itinerary
    // Background
    drawRoundedRect(0, 0, 210, 297, 0, colors.background);
    
    // Header section with decorative border
    drawRoundedRect(15, 15, 180, 50, 8, colors.white);
    doc.setDrawColor(...colors.primary);
    doc.setLineWidth(2);
    doc.roundedRect(15, 15, 180, 50, 8, 8, 'S');
    ...
    doc.text(weddingTitle, 105, 35, { align: 'center' });
    ...
    doc.text(weddingDate, 105, 50, { align: 'center' });
    ...
    doc.text('Ceremony & Reception', 105, 70, { align: 'center' });
    
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.setTextColor(...colors.text);
    doc.text(venue, 105, 80, { align: 'center' });
    doc.text(time, 105, 87, { align: 'center' });
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...colors.primary);
    doc.text('Wedding Day Schedule', 20, yPosition);
    yPosition += 15;
    ...
    dayOfItems.forEach((item, index) => {
      ...
      const itemHeight = 25 + (item.description ? 8 : 0) + (item.location || item.responsible ? 6 : 0);
      ...
      if (item.duration > 0) {
        ...
        drawRoundedRect(150, yPosition + 2, durationWidth + 6, 10, 5, colors.accent);
        doc.setFontSize(8);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(...colors.white);
        doc.text(durationText, 153, yPosition + 8);
      }
      ...
      if (item.description) {
        ...
        doc.setFontSize(9);
        doc.setTextColor(...colors.text);
        const descLines = wrapText(item.description, 100);
        descLines.forEach((line, lineIndex) => {
          doc.text(line, 45, yPosition + 28 + (lineIndex * 4));
        });
        yPosition += descLines.length * 4;
      }

      // Location and Responsible (simplified for guest version)
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...colors.text);
    doc.text('Thank you for celebrating with us!', 105, footerY, { align: 'center' });
    doc.text('Generated by Bidi Wedding Planner', 105, footerY + 5, { align: 'center' });

    // Save the PDF
    const fileName = `${weddingTitle.replace(/\s+/g, '_')}_Wedding_Schedule.pdf`;
    doc.save(fileName);
  };
  ...
  const generateComprehensivePDF = () => {
    ...
    dayOfItems.forEach((item, index) => {
      ...
      if (item.location || item.responsible) {
        ...
        let details = '';
        if (item.location) details += `Location: ${item.location}`;
        if (item.responsible) details += details ? ` | Person: ${item.responsible}` : `Person: ${item.responsible}`;
        doc.text(details, 45, yPosition + 28);
      }
