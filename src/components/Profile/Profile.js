...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        if (profile.role === "business") {
          ...
          const { data: individualProfile, error: individualError } =
            await supabase
    ...
    const fetchProfileImages = async (userId) => {
      try {
        ...
        const profileImage = data.find(
          (photo) => photo.photo_type === "profile"
        );
        const portfolioImages = data.filter(
  ...
  const handleFileChange = async (event, type) => {
    ...
    if (type === "profile") {
      ...
      const uploadedUrl = await handleUpload(file, type);
      setPortfolioPic((prevPics) =>
        prevPics.map((pic) => (pic === previewURL ? uploadedUrl : pic))
      );
    }
  };
          ...
          <>
            ...
            <div className="portfolio-container">
              ...
              <button
                className="edit-profile-button"
                onClick={() =>
                  document.getElementById("portfolioPicInput").click()
                }
              >
