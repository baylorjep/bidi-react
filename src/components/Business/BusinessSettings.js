...
const [loadingAiBids, setLoadingAiBids] = useState(false);
  ...
  const fetchAiGeneratedBids = async () => {
    ...
    try {
      setLoadingAiBids(true);
      
      // Fetch bids that were generated by AI (is_autobid = true) for this business
                    ...
                    <>
                      ...
                      <div className="settings-row">
                        <div>
                          ...
                          <div className="settings-desc">Control whether your AI is actively generating bids for new requests.</div>
                        </div>
                        <div className="settings-control">
                          <div className="d-flex gap-2 align-items-center">
                            <span className={`badge ${autobidPaused ? 'bg-warning' : 'bg-success'} fs-6`}>
                              {autobidPaused ? 'Paused' : 'Live'}
                            </span>
                            <button
                          ...
                          <small className="text-muted d-block mt-2">
                            {autobidPaused 
                              ? "Your AI is paused and won't generate new bids. Resume to start generating bids again."
                              : "Your AI is live and will automatically generate bids for new requests."
                            }
                          </small>
                        </div>
                      </div>
                      ...
                      <div className="settings-row">
                        <div>
                          ...
                          <div className="settings-desc">Review and manage bids that were automatically generated by your AI while you were away.</div>
                        </div>
                        <div className="settings-control" style={{ width: '100%' }}>
                          {loadingAiBids ? (
                            <div className="text-center py-3">
                              <i className="fas fa-spinner fa-spin me-2"></i>
                              Loading AI-generated bids...
                            </div>
                          ) : aiGeneratedBids.length > 0 ? (
                            <div className="ai-bids-container">
                              <div className="d-flex justify-content-between align-items-center mb-3">
                                <h6 className="mb-0">Recent AI-Generated Bids ({aiGeneratedBids.length})</h6>
                                <button
                              ...
                              <div className="ai-bids-list">
                                {aiGeneratedBids.map((bid) => (
                                  <div key={bid.id} className="ai-bid-item mb-3 p-3 border rounded">
                                    <div className="d-flex justify-content-between align-items-start mb-2">
                                      <div>
                                        <strong>${bid.bid_amount}</strong>
                                        <span className="text-muted ms-2">• {bid.requests?.event_type || 'Event'}</span>
                                      </div>
                                      <span className="badge bg-info">AI Generated</span>
                                    </div>
                                    <div className="mb-2">
                                      <small className="text-muted">
                                        Generated on {new Date(bid.created_at).toLocaleDateString()}
                                      </small>
                                    </div>
                                    <div className="mb-3">
                                      <p className="mb-1" style={{ fontSize: '0.9rem' }}>
                                        {bid.bid_description?.substring(0, 150)}
                                        {bid.bid_description?.length > 150 && '...'}
                                      </p>
                                    </div>
                                    <div className="d-flex gap-2">
                                      <button
                                        className="btn btn-sm btn-primary"
                                        onClick={() => navigate(`/edit-bid/${bid.id}`)}
                                      >
                            ...
                            <div className="text-center py-4 text-muted">
                              ...
                              <p className="mb-0">No AI-generated bids yet</p>
                              <small>When your AI generates bids while you're away, they'll appear here for review.</small>
                            </div>
                          )}
                        </div>
                      </div>
