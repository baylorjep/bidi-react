...
const BusinessDashboard = () => {
  ...
  useEffect(() => {
    const fetchBusinessDetails = async () => {
      try {
        ...
        const { data: verificationApp, error: verificationError } =
          await supabase
        ...
        const isPending =
          (!profile.membership_tier || profile.membership_tier === "") &&
          verificationApp !== null;

        setIsVerified(
          profile.membership_tier === "Plus" ||
            profile.membership_tier === "Verified"
        );
        ...
        const { data: profileDetails, error: profileDetailsError } =
          await supabase
            .from("business_profiles")
            .select(
              "business_name, stripe_account_id, id, down_payment_type, amount, minimum_price, business_category, membership_tier, verification_pending, story"
            )
            .eq("id", user.id)
            .single();
        ...
        if (profileDetails) {
          ...
          if (
            profileDetails.down_payment_type !== null &&
            profileDetails.amount !== null
          ) {
          ...
          const bidsWithRequestData = await Promise.all(
            bidsData.map(async (bid) => {
              ...
              try {
                ...
                console.log("DJ request search result:", {
                  data: djRequest,
                  error: djError,
                }); // Debug log
                ...
                console.log(
                  "Searched tables: dj_requests, catering_requests, beauty_requests, florist_requests, photography_requests, videography_requests, requests"
                );
                return {
                  ...bid,
    ...
    if (user) {
      ...
      if (error) {
        ...
        const validCoupon = coupons.find(
          (coupon) => new Date(coupon.expiration_date) > now
        );
  ...
  const fetchUnviewedBidCount = async () => {
    try {
      ...
      const hasContactedField =
        columnsData && columnsData.length > 0 && "contacted" in columnsData[0];
  ...
  const handleGenerateCoupon = async () => {
    ...
    const now = new Date();
    const validCoupon = existingCoupons?.find(
      (coupon) => new Date(coupon.expiration_date) > now
    );
    ...
    <div className="business-dashboard text-center">
      ...
      <div className="container mt-4">
        <div className="row justify-content-center">
          ...
          {isAdmin && (
            <div
              className="col-lg-10 col-md-12 col-sm-12 d-flex flex-column"
              style={{ marginTop: "20px" }}
            >
              <div className="position-relative">
                {" "}
                {/* Wrap the button in a div with position-relative */}
                <button
                ...
                {stripeError && (
                  <button
                    ...
                    onClick={() => {
                      if (
                        window.confirm(
                          "Are you sure you want to reset your Stripe connection? You will need to set it up again to receive payments."
                        )
                      ) {
                    ...
                    {bid.request_type === "Normal Request" ? (
                      <div>
                        <p>
                          ...
                          {bid.request_data?.location}
                        </p>
                        <p>
                          <strong>
                            {bid.request_data?.end_date ? "Start Date" : "Date"}
                            :
                          </strong>{" "}
                          {bid.request_data?.service_date}
                        </p>
                      ...
                      <div>
                        <p>
                          ...
                          {bid.request_data?.event_type}
                        </p>
                        {bid.request_data?.date_flexibility === "specific" ? (
                          <p>
                            <strong>Event Date: </strong>
                            {new Date(
                              bid.request_data?.start_date
                            ).toLocaleDateString()}
                          </p>
                          ...
                          <p>
                            ...
                            {bid.request_data?.date_timeframe === "3months"
                              ? "Within 3 months"
                              : bid.request_data?.date_timeframe === "6months"
                              ? "Within 6 months"
                              : bid.request_data?.date_timeframe === "1year"
                              ? "Within 1 year"
                              : bid.request_data?.date_timeframe === "more"
                              ? "More than 1 year"
                              : "Not specified"}
                          </p>
                        )}
                        ...
                        <p>
                          ...
                          {bid.request_data?.num_people}
                        </p>
                        <p>
                          <strong>Duration: </strong>
                          {bid.request_data?.duration}
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p>
                          ...
                          {typeof bid.request_data?.music_preferences ===
                          "object"
                            ? Object.keys(
                        ...
                        <p>
                          ...
                          {bid.request_data?.equipment_needed ? "Yes" : "No"}
                        </p>
                        <p>
                          <strong>Duration: </strong>
                          {bid.request_data?.event_duration} hours
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        <p>
                          ...
                          {bid.request_data?.estimated_guests}
                        </p>
                        <p>
                          <strong>Cuisine Type: </strong>
                          {Array.isArray(bid.request_data?.food_preferences)
                            ? bid.request_data?.food_preferences.join(", ")
                            : typeof bid.request_data?.food_preferences ===
                              "object"
                            ? Object.keys(
                                bid.request_data?.food_preferences
                              ).join(", ")
                            : bid.request_data?.food_preferences}
                        </p>
                      </div>
                    ...
                    <div className="bid-details-dashboard">
                      ...
                      <p className="card-text">
                        {bid.viewed ? (
                          ...
                          <span className="not-viewed-status">
                            Not viewed yet
                          </span>
                        )}
                      </p>
