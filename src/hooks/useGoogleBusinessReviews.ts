...
export const useGoogleBusinessReviews = (businessId: string) => {
  ...
  const fetchReviews = useCallback(async (accountId: string, locationId: string) => {
    ...
    try {
      ...
      const reviewsToInsert = data.reviews.map((review: any) => ({
